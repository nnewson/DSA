...
void assert_rb_properties(const RedBlackTree<K, V>& tree) {
    if (tree.root_node() == tree.nil_node()) return;
...
protected:
    IntTree tree;
    void SetUp() override { tree.add(10, "ten"); }
};
...
TEST(DuplicateInsert, UpdatePreservesRbProperties) {
    IntTree tree;
    for (int k : {10, 5, 15}) tree.add(k, std::to_string(k));
    tree.add(5, "updated");
    assert_rb_properties(tree);
...
TEST(MultipleInserts, AscendingOrder) {
    IntTree tree;
    for (int k = 1; k <= 7; ++k) tree.add(k, std::to_string(k));
...
TEST(MultipleInserts, DescendingOrder) {
    IntTree tree;
    for (int k = 7; k >= 1; --k) tree.add(k, std::to_string(k));
    EXPECT_EQ(tree.size(), 7u);
    assert_rb_properties(tree);
}
...
TEST(MultipleInserts, MixedOrder) {
    ...
    for (int k : keys) tree.add(k, std::to_string(k));
    EXPECT_EQ(tree.size(), keys.size());
    for (int k : keys) EXPECT_EQ(tree.find(k)->get(), std::to_string(k));
    assert_rb_properties(tree);
}
...
TEST(MultipleInserts, HundredAscending) {
    IntIntTree tree;
    for (int k = 1; k <= 100; ++k) tree.add(k, k);
...
TEST(MultipleInserts, HundredDescending) {
    IntIntTree tree;
    for (int k = 100; k >= 1; --k) tree.add(k, k);
    EXPECT_EQ(tree.size(), 100u);
    assert_rb_properties(tree);
}
...
TEST(InsertFixupCases, UncleRedRecoloring) {
    IntTree tree;
    for (int k : {10, 5, 15, 3}) tree.add(k, std::to_string(k));
    assert_rb_properties(tree);
    auto col = nc(tree, tree.root_node());
    EXPECT_EQ(col, Colour::Black);
}
...
TEST(InsertFixupCases, RecoloringPropagatesToRoot) {
    IntTree tree;
    for (int k : {10, 5, 15, 3, 7, 12, 20, 1}) tree.add(k, std::to_string(k));
    assert_rb_properties(tree);
}

TEST(InsertFixupCases, LeftRotationAfterRightInsertion) {
    IntTree tree;
    for (int k : {10, 5, 15, 7}) tree.add(k, std::to_string(k));
    assert_rb_properties(tree);
}

TEST(InsertFixupCases, RightRotationAfterLeftInsertion) {
    IntTree tree;
    for (int k : {10, 5, 15, 12}) tree.add(k, std::to_string(k));
    assert_rb_properties(tree);
}

TEST(InsertFixupCases, DeepLeftSpineForcesRotations) {
    IntIntTree tree;
    for (int k = 1; k <= 15; ++k) tree.add(k, k);
    assert_rb_properties(tree);
}

TEST(InsertFixupCases, DeepRightSpineForcesRotations) {
    IntIntTree tree;
    for (int k = 15; k >= 1; --k) tree.add(k, k);
    assert_rb_properties(tree);
}
...
TEST(Find, DeepNode) {
    IntTree tree;
    for (int k : {50, 25, 75, 10, 30, 60, 90}) tree.add(k, std::to_string(k));
...
TEST(Find, SmallestKey) {
    IntTree tree;
    for (int k : {50, 25, 75, 10, 30}) tree.add(k, std::to_string(k));
    EXPECT_EQ(tree.find(10)->get(), "10");
}
...
TEST(Find, AllInsertedKeys) {
    IntIntTree tree;
    for (int k = 1; k <= 20; ++k) tree.add(k, k * 10);
    for (int k = 1; k <= 20; ++k) EXPECT_EQ(tree.find(k)->get(), k * 10);
}
...
TEST(Iter, SortedOrder) {
    IntTree tree;
    for (int k : {30, 10, 50, 5, 20, 40, 60}) tree.add(k, std::to_string(k));
    auto pairs = tree.collect_inorder();
    std::vector<int> keys;
...
TEST(Iter, ValuesMatchKeys) {
    IntIntTree tree;
    for (int k : {30, 10, 50, 5, 20}) tree.add(k, k * 10);
    auto pairs = tree.collect_inorder();
    ASSERT_EQ(pairs.size(), 5u);
    EXPECT_EQ(pairs[0].first, 5);   EXPECT_EQ(pairs[0].second, 50);
    EXPECT_EQ(pairs[1].first, 10);  EXPECT_EQ(pairs[1].second, 100);
    EXPECT_EQ(pairs[2].first, 20);  EXPECT_EQ(pairs[2].second, 200);
    EXPECT_EQ(pairs[3].first, 30);  EXPECT_EQ(pairs[3].second, 300);
    EXPECT_EQ(pairs[4].first, 50);  EXPECT_EQ(pairs[4].second, 500);
}
...
TEST(Iter, AfterRemoves) {
    IntTree tree;
    for (int k : {10, 5, 15, 3, 7}) tree.add(k, std::to_string(k));
    tree.remove(5);
    tree.remove(15);
    auto pairs = tree.collect_inorder();
...
TEST(Iter, AscendingInsertionOrder) {
    ...
    for (int k = 1; k <= 7; ++k) tree.add(k, std::to_string(k));
    auto pairs = tree.collect_inorder();
    for (int i = 0; i < 7; ++i) EXPECT_EQ(pairs[i].first, i + 1);
}
...
TEST(Iter, LargeTree) {
    IntIntTree tree;
    for (int k = 0; k < 500; ++k) tree.add(k, k);
...
TEST(DeleteFixupCases, Case1SiblingIsRed) {
    IntTree tree;
    for (int k : {10, 5, 15, 3, 7, 12, 20}) tree.add(k, std::to_string(k));
    tree.remove(3);
    assert_rb_properties(tree);
}
...
TEST(DeleteFixupCases, Case3BlackSiblingFarChildRed) {
    IntTree tree;
    for (int k : {10, 5, 15, 20}) tree.add(k, std::to_string(k));
    tree.remove(5);
    assert_rb_properties(tree);
}
...
TEST(DeleteFixupCases, MultipleFixupIterations) {
    IntTree tree;
    for (int k : {20, 10, 30, 5, 15, 25, 35, 3, 7}) tree.add(k, std::to_string(k));
    tree.remove(35);
    tree.remove(30);
    assert_rb_properties(tree);
}
...
TEST(DeleteFixupCases, SuccessorNotDirectChild) {
    IntTree tree;
    for (int k : {20, 10, 30, 25, 35, 23}) tree.add(k, std::to_string(k));
    tree.remove(20);
    EXPECT_FALSE(tree.find(20).has_value());
    for (int k : {10, 30, 25, 35, 23}) EXPECT_EQ(tree.find(k)->get(), std::to_string(k));
    assert_rb_properties(tree);
}
...
TEST(DeleteAll, Ascending) {
    IntTree tree;
    for (int k = 1; k <= 10; ++k) tree.add(k, std::to_string(k));
    for (int k = 1; k <= 10; ++k) {
...
TEST(DeleteAll, ArbitraryOrder) {
    IntTree tree;
    for (int k : {50, 25, 75, 12, 37, 62, 87}) tree.add(k, std::to_string(k));
...
TEST(InterleavedOps, DeleteEvensKeepOdds) {
    IntIntTree tree;
    for (int k = 1; k <= 20; ++k) tree.add(k, k);
    for (int k = 2; k <= 20; k += 2) tree.remove(k);
    EXPECT_EQ(tree.size(), 10u);
    assert_rb_properties(tree);
    for (int k = 1; k <= 20; k += 2) EXPECT_EQ(tree.find(k)->get(), k);
    for (int k = 2; k <= 20; k += 2) EXPECT_FALSE(tree.find(k).has_value());
}
...
TEST(InterleavedOps, BulkInsertThenSelectiveDelete) {
    IntTree tree;
    for (int k = 1; k <= 15; ++k) tree.add(k, std::to_string(k));
...
TEST(KeyTypes, NegativeIntegerKeys) {
    IntTree tree;
    for (int k : {-5, -3, -1, 0, 2, 4}) tree.add(k, std::to_string(k));
    EXPECT_EQ(tree.size(), 6u);
    for (int k : {-5, -3, -1, 0, 2, 4}) EXPECT_EQ(tree.find(k)->get(), std::to_string(k));
    assert_rb_properties(tree);
}
...
TEST(Len, DecrementsOnEachDelete) {
    IntIntTree tree;
    for (int i = 1; i <= 5; ++i) tree.add(i, i);
    for (int i = 1; i <= 5; ++i) {
        tree.remove(i);
...
TEST(Len, ZeroAfterAllDeleted) {
    IntIntTree tree;
    for (int k = 0; k < 10; ++k) tree.add(k, k);
    for (int k = 0; k < 10; ++k) tree.remove(k);
    EXPECT_EQ(tree.size(), 0u);
}
...
TEST(BSTOrdering, InorderSortedAfterInserts) {
    ...
    auto pairs = collect_inorder(tree);
    std::sort(keys.begin(), keys.end());
    for (std::size_t i = 0; i < keys.size(); ++i) EXPECT_EQ(pairs[i].first, keys[i]);
}
...
TEST(BSTOrdering, InorderSortedAfterDeletes) {
    ...
    for (std::size_t i = 0; i < remaining.size(); ++i) EXPECT_EQ(pairs[i].first, remaining[i]);
}
...
TEST(ParentPointers, ConsistentAfterRotations) {
    IntIntTree tree;
    for (int k = 1; k <= 10; ++k) tree.add(k, k);
    assert_parent_pointers(tree);
}
...
TEST(Stress, ThousandAscendingInserts) {
    IntIntTree tree;
    for (int k = 0; k < 1000; ++k) tree.add(k, k);
...
TEST(Stress, Insert500DeleteEvens) {
    IntIntTree tree;
    for (int k = 0; k < 500; ++k) tree.add(k, k);
    for (int k = 0; k < 500; k += 2) tree.remove(k);
    EXPECT_EQ(tree.size(), 250u);
    assert_rb_properties(tree);
    for (int k = 1; k < 500; k += 2) EXPECT_EQ(tree.find(k)->get(), k);
}

TEST(Stress, InsertThenDeleteAll1000) {
    IntIntTree tree;
    for (int k = 0; k < 1000; ++k) tree.add(k, k);
    for (int k = 0; k < 1000; ++k) EXPECT_TRUE(tree.remove(k));
    EXPECT_EQ(tree.size(), 0u);
}

TEST(Stress, FindAllInLargeTree) {
    IntIntTree tree;
    for (int k = 0; k < 500; ++k) tree.add(k, k * 2);
    for (int k = 0; k < 500; ++k) EXPECT_EQ(tree.find(k)->get(), k * 2);
    for (int k = 500; k < 600; ++k) EXPECT_FALSE(tree.find(k).has_value());
}
...
TEST(EdgeCases, DeleteMinimumKey) {
    IntTree tree;
    for (int k : {20, 10, 30, 5, 15}) tree.add(k, std::to_string(k));
    tree.remove(5);
    EXPECT_FALSE(tree.find(5).has_value());
    assert_rb_properties(tree);
}
...
TEST(EdgeCases, ManyDuplicatesNoSizeGrowth) {
    IntTree tree;
    tree.add(42, "v1");
    for (int i = 0; i < 100; ++i) tree.add(42, "v" + std::to_string(i + 2));
    EXPECT_EQ(tree.size(), 1u);
...
TEST(EdgeCases, DeleteAllThenReinsert) {
    IntTree tree;
    for (int k = 1; k <= 5; ++k) tree.add(k, std::to_string(k));
    for (int k = 1; k <= 5; ++k) tree.remove(k);
    EXPECT_EQ(tree.size(), 0u);
    for (int k = 10; k < 15; ++k) tree.add(k, std::to_string(k));
    EXPECT_EQ(tree.size(), 5u);
    for (int k = 10; k < 15; ++k) EXPECT_EQ(tree.find(k)->get(), std::to_string(k));
    assert_rb_properties(tree);
}
